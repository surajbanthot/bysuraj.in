...
export default function F1Car() {
    ...
    const startSequence = () => {
        ...
        // Animation Duration is 5s total.
        // 0-15% (0.75s): Drive In
        // 15-85% (3.5s wait): Wait Phase
        // Start burnout at 1.5s, for 2 seconds.

        burnStartRef.current = setTimeout(() => setIsBurning(true), 1500);
        burnEndRef.current = setTimeout(() => setIsBurning(false), 3500);

        // Message "I'm fast AF boy!!" just before takeoff
        msgStartRef.current = setTimeout(() => setShowFastMsg(true), 1500);
        msgEndRef.current = setTimeout(() => setShowFastMsg(false), 3000);
    };
    ...
    const handleAnimationEnd = () => {
        ...
        // Schedule next run (random 10-30s)
        const nextDelay = Math.random() * 20000 + 10000;

        clearAllTimers();
        timerRef.current = setTimeout(startSequence, nextDelay);
    };
    ...
    const handleCrash = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        // Start sink animation at 4s
        setTimeout(() => setIsSinking(true), 4000);

        // Full reset at 6.5s (allow sink to finish)
        setTimeout(handleAnimationEnd, 6500);
    };
            ...
            {isCrashed && (
                ...
                >
                    <div className={isSinking ? "animate-sink" : ""}>
                        ...
                        <svg viewBox="0 0 320 320" shapeRendering="crispEdges" className="h-40 w-40 drop-shadow-2xl" style={{ overflow: "visible" }}>
